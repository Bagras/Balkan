...
func test_content_loads(db: ContentDB) -> void:
	...
	equal(db.events.size(), 129, "всего событий (47 из .docx + 82 дополнительных)")
	equal(db.events_of_kind("random").size(), 76, "случайных событий")
	equal(db.events_of_kind("chain").size(), 36, "шагов цепочек")
	equal(db.chain_starter_count(), 9, "цепочек в игре")
